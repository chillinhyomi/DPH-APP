...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  async function login(email: string, password: string): Promise<string | null> {
    try {
      ...
      if (res.error) {
        ...
        return res.error.message || "로그인 중 오류가 발생했습니다.";
      }
      if (!res.data.user) {
        return "로그인에 실패했습니다. 이메일과 비밀번호를 확인하세요.";
      }
      const u = res.data.user;
      setCurrentUser({ uid: u.id, name: (u.user_metadata as any)?.name || (u.email?.split("@")[0] || ""), email: u.email || "", avatarColor: DANCER_COLORS[0] });
  ...
  async function register(name: string, email: string, password: string): Promise<string | null> {
    try {
      ...
      if (res.error) {
        ...
        return res.error.message || "회원가입 중 오류가 발생했습니다.";
      }
      const u = res.data.user;
      if (u) {
        setCurrentUser({ uid: u.id, name, email: u.email || email, avatarColor: DANCER_COLORS[0] });
        setAuthLoading(false);
        setAuthMessage(null);
      }
      return null;
  ...
  useEffect(() => {
    ...
    const { data: sub } = supabase.auth.onAuthStateChange((event, session) => {
      if (session?.user) {
        ...
        setCurrentUser(null);
        setAuthLoading(false);
        // if the change was an explicit sign out or session lost, show message
  ...
  return (
    <AppContext.Provider value={{ currentUser, projects, members, formations, musicFiles, notices, scheduleEvents, qnaPosts, isAuthenticated: !!currentUser, authLoading, authMessage, login, register, logout, updateUser, createProject, joinProject, updateFormations, updateMembers, uploadMusic, removeMusic, renameProject, deleteProject, toggleFavorite, addNotice, addComment, addScheduleEvent, createQnAPost, updateQnAPost, deleteQnAPost, addQnAReply, incrementQnAViews, DANCER_COLORS }}>
      {children}
    </AppContext.Provider>
  );
}
